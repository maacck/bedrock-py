...
- 带依赖解析和生命周期钩子的模块注册表
- 依赖注入容器，支持 singleton/transient/scoped 服务生命周期
- 用于结构化调用/响应扩展点的 Hook 系统
...
| `bedrock.di.container` | `Container` | 依赖注入 |
| `bedrock.hooks.hooks` | `HookRegistry` | Hook specs 和 implementations |
